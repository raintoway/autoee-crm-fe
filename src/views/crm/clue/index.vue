...
import { ClueVO } from '@/api/crm/clue'
...
/** 转化为客户 */
const handleTransform = async (clue: ClueApi.ClueVO) => {
  await message.confirm(`确定将【${clue.name}】转化为客户吗？`)
  await ClueApi.transformClue(clue.id)
  message.success(`转化客户【${clue.name}】成功`)
  await getList()
}
