    ...
    <el-collapse v-model="activeNames">
      <el-collapse-item name="contractInfo">
        ...
        <el-descriptions :column="4">
          <el-descriptions-item label="订单编号">{{ contract.no }}</el-descriptions-item>
          <el-descriptions-item label="订单名称">{{ contract.name }}</el-descriptions-item>
          ...
          <el-descriptions-item label="订单开始时间">
            {{ formatDate(contract.startTime) }}
          </el-descriptions-item>
          <el-descriptions-item label="订单结束时间">
...
defineOptions({ name: 'BpmContractDetailsInfo' })
...
const getContractData = async () => {
  try {
    contract.value = await ContractApi.getContract(props.id )
  } finally {
  }
}
</script>
